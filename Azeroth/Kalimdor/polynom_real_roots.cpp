// полином вида x^n+k[n-1]*x^(n-1)+k[n-2]*x^(n-2)+...+k[1]*x+k[0] со старшим коэффициентом, равным единице
...
int* currentRootsCount  // сформированные в вызывающей процедуре
	...
	double major=0;  // аргумент полинома, равносильный его бесконечно большому значению
	...
	for(int i = 0; i <= currentRootsCount[level-1]; i++) {
		int signLeft, signRight;  // знаки текущего A-полинома на левой и правой границе интервала монотонности
		double edgeLeft, edgeRight;  // предварительная левая и правая границы интервала поиска
		double edgeNegativ, edgePositiv;  // границы интервала монотонности, несущие информацию о знаке полинома на них
		...
		double rb = polinom(level, edgeLeft, A[level]);  // значение текущего A-полинома на левой границе
		...
		// если знаки полинома на границах интервала монотонности совпадают, то корня нет

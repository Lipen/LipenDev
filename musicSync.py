...
if len(absentDirsSlave) > 0:
		...
		for item in absentDirsSlave:
			print_safe(item.encode('utf').decode('utf', 'replace'))

while 1:
	e = input('Exit or continue...').lower()
	if e in ['c', 'cont', 'continue', 'go', 'next', 'second']:
		break
	elif e in ['e', 'exit', 'quit', 'first', 'stop']:
		exit()
	else:
		continue
...
if 'd' in e:
	if len(deleteList) > 0:
		...
		for item in deleteList:
			os.remove(item)
			print_safe('  {}  has been deleted'.format(item))
		print('Removing done.')
	else:
...
if 'c' in e:
	if len(copyList) > 0:
		...
		for item in copyList:
			...
			print_safe('  {}  has been copied'.format(item[2]))
		print('Copying done.')
	else:
